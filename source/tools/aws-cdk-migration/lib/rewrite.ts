...
function extractBarrelConstructImport(
  ...
  const constructName = importedNames.find((name) => (name.propertyName ?? name.name).text === 'Construct' || (name.propertyName ?? name.name).text === 'IConstruct');
  if (constructName) {
    ...
    if (constructName.name.text === 'Construct') {
      return {
      ...
      return {
        searchName: constructName.name.text,
        replacementName: constructName.name.text,
        newImport: `import { IConstruct${aliasStatement} } from 'constructs';`,
      };
    }
  }
  return undefined;
