...
machine). Specifying both this and enrichmentStateMachine causes an error.
Default - undefined
...
pipeLogProps causes an error.

|pipeLogProps? |https://docs.aws.amazon.com/cdk/api/v2/docs/aws-cdk-lib.aws_logs.LogGroupProps.html[`logs.LogGroupProps`] |Default
behavior is for this construct to create a new CloudWatch Logs log group for the pipe.
...
image::aws-dynamodbstreams-pipes-stepfunctions.png["Diagram showing data flow between AWS services including DynamoDB, EventBridge Pipe, CloudWatch log groups, State Machine and an IAM role.",scaledwidth=100%]
