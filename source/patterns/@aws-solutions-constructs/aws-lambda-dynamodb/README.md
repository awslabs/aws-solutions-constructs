...
|existingLogGroup?|[`logs.LogGroup`](https://docs.aws.amazon.com/cdk/api/latest/docs/@aws-cdk_aws-logs.LogGroup.html)|Existing instance of a Log Group object.|
|logGroupProps?|[`logs.LogGroupProps`](https://docs.aws.amazon.com/cdk/api/latest/docs/@aws-cdk_aws-logs.LogGroupProps.html)|Optional user-provided properties to override the default properties for the new Log Group. `encryptionKey`, `logGroupName`, `removalPolicy` and `retention` are set by the pattern, so any values for those properties supplied here will be overrriden. If properties are empty, it will deploy with the default properties of a default master key, generated log group name, retain removal policy, and two year retention period.|
...
|logGroup?|[`logs.LogGroup`](https://docs.aws.amazon.com/cdk/api/latest/docs/@aws-cdk_aws-logs.LogGroup.html)|Returns an instance of logs.LogGroup created by the construct.|
