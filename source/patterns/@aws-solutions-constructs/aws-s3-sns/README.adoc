...
|Optional - existing instance of SNS topic object, providing both this and `topicProps` will cause an error. If the SNS Topic is encrypted with a Customer-Managed KMS Key, the key must be specified in the `existingTopicEncryptionKey` property.
...
image::aws-s3-sns.png["Diagram showing the S3 bucket and SNS topic created by the construct",scaledwidth=100%]
