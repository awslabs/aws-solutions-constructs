...
Topic is encrypted using SQS_MANAGED encryption. NOTE - other constructs
with SQS queues default to AWS managed KMS key encryption, but an SNS
subscription does not work with that configuration. For a completely
...
image::aws-sns-sqs.png["Diagram showing the SNS topic, and SQS queue and dlg created by the construct",scaledwidth=100%]
