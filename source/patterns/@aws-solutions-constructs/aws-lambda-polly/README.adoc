...
image::aws-lambda-polly.png["Diagram showing the Lambda function, Amazon Polly service, and IAM role created by the construct",scaledwidth=100%]

**Resources created:**

* AWS Lambda Function
* IAM Role with permissions to call Amazon Polly SynthesizeSpeech API

**Default Implementation when asyncJobs = true**

image::aws-lambda-polly-async.png["Diagram showing the Lambda function, S3 bucket for audio output (when asyncJobs is true), SNS topic, Amazon Polly service, and IAM roles created by the construct",scaledwidth=100%]

**Resources created:**

* AWS Lambda Function
* Amazon S3 Bucket for audio output
* Amazon S3 Bucket for access logs
* Amazon SNS Topic for completion notifications
* IAM Role with permissions to call Amazon Polly APIs (synchronous and asynchronous), read/write to S3 bucket, and publish to SNS topic

== Example Lambda Function Implementation

The Lambda function can call Amazon Polly to convert text to speech. Here's an example implementation:
...
export const handler = async (event: any) => {
  const command = new SynthesizeSpeechCommand({
    Text: event.text || "Hello from Amazon Polly",
    OutputFormat: "mp3",
    VoiceId: "Joanna"
  });
  ...
  // AudioStream is returned as a readable stream
...
=== Asynchronous Mode with Custom Bucket and Topic
...
const existingBucket = s3.Bucket.fromBucketName(this, 'ExistingBucket', 'my-audio-bucket');
const existingTopic = sns.Topic.fromTopicArn(this, 'ExistingTopic', 'arn:aws:sns:us-east-1:123456789012:my-topic');
...
existing_bucket = s3.Bucket.from_bucket_name(self, 'ExistingBucket', 'my-audio-bucket')
existing_topic = sns.Topic.from_topic_arn(self, 'ExistingTopic', 'arn:aws:sns:us-east-1:123456789012:my-topic')
...
LambdaToPolly(self, 'LambdaToPollyPattern',
    ...
    existing_topic_obj=existing_topic
)
----
====
