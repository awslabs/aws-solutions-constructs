...
|existingVpc?|[`ec2.IVpc`](https://docs.aws.amazon.com/cdk/api/latest/docs/@aws-cdk_aws-ec2.IVpc.html)|Optional, existing VPC into which this Lambda function and Elasticsearch cluster should be deployed. Lambda function and ElasticSearch service are deployed in same VPC, the Lambda function and ElasticSearch service will use ENIs in the VPC to access network resources. This construct sets Elasticsearch cluster deployment type as production, deploying using exactly 3 AZs. In that case, the provided VPC need to have minimum 3 AZs. If an existing VPC is provided, the `deployVpc` property cannot be `true`. This uses `ec2.IVpc` to allow clients to supply VPCs that exist outside the stack using the [`ec2.Vpc.fromLookup()`](https://docs.aws.amazon.com/cdk/api/latest/docs/@aws-cdk_aws-ec2.Vpc.html#static-fromwbrlookupscope-id-options) method.|
...
|deployVpc?|`boolean`|Whether to create a new VPC based on `vpcProps` into which to deploy the Lambda function. Setting this to true will deploy the minimal, most private VPC to run the pattern:<ul><li> One isolated subnet in each Availability Zone used by the CDK program</li><li>`enableDnsHostnames` and `enableDnsSupport` will both be set to true</li></ul>If this property is `true` then `existingVpc` cannot be specified. Defaults to `false`.|
...
|vpc?|[`ec2.IVpc`](https://docs.aws.amazon.com/cdk/api/latest/docs/@aws-cdk_aws-ec2.IVpc.html)|Returns an interface on the VPC used by the Lambda function (if any). This may be a VPC created by the pattern or the VPC supplied to the pattern constructor.|
