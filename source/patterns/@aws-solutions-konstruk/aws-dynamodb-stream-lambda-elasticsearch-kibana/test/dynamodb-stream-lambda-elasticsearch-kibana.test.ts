...
test('check getter methods', () => {
  ...
  expect(construct.elasticsearchDomain()).toBeInstanceOf(CfnDomain);
  expect(construct.identityPool()).toBeInstanceOf(CfnIdentityPool);
  expect(construct.userPool()).toBeInstanceOf(UserPool);
  expect(construct.userPoolClient()).toBeInstanceOf(UserPoolClient);
  expect(construct.cloudwatchAlarms()).toHaveLength(9);
});
