 ...
 * @param {objet} defaultProps the prescriptive defaults for the pattern.
...
export function flagOverriddenDefaults(defaultProps: object, userProps: object) {
  ...
  for (let i = 0; i < ((overrides !== undefined) ? overrides.length : 0); i++) {
    ...
    log.warn('AWS_KONSTRUK_WARNING: ', `An override has been provided for the property: ${path}.` + details);
  }
}
