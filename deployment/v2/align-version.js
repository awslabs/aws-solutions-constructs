...
const awsCdkLibVersion = "2.266.0";

for (const file of process.argv.splice(4)) {
  const pkg = JSON.parse(fs.readFileSync(file).toString());

  if (
    pkg.version !== nullVersionMarker &&
    pkg.version !== targetSolutionsConstructsVersion
  ) {
    throw new Error(
      `unexpected - all package.json files in this repo should have a version of ${nullVersionMarker} or ${targetSolutionsConstructsVersion}: ${file}`,
    );
  }
  ...
  pkg.peerDependencies = updateDependencyVersionNumbers(
    pkg.peerDependencies || {},
    "^",
  );
  pkg.devDependencies = updateDependencyVersionNumbers(
    pkg.devDependencies || {},
  );
...
function updateDependencyVersionNumbers(section, orBetter = "") {
  let newdependencies = {};
  for (const [name, version] of Object.entries(section)) {
    if (name.startsWith("@aws-solutions-constructs")) {
      newdependencies[name] = version.replace(
        nullVersionMarker,
        targetSolutionsConstructsVersion,
      );
    } else if (
