...
const expectedBlueOakLibraries = process.argv[3];
...
if (blueOakPackages.length > 0) {
  CheckAndLogBlueOakLicenses(blueOakPackages, expectedBlueOakLibraries);
}
...
// the future
function CheckAndLogBlueOakLicenses(packages, expectedBlueOakLibraries) {
...
Blue Oak Model License - https://blueoakcouncil.org/license/1.0.0\n`;
  ...
  const minimatchTemplate =  `—— minimatch (multiple versions) ———
Copyright Isaac Z. Schlueter <i@izs.me>
https://github.com/isaacs/minimatch
Blue Oak Model License - https://blueoakcouncil.org/license/1.0.0\n`;

  const minipassTemplate =  `—— minipass @ ${versionPlaceholder} ———
Copyright Isaac Z. Schlueter <i@izs.me>
https://github.com/isaacs/minipass
Blue Oak Model License - https://blueoakcouncil.org/license/1.0.0\n`;

  console.log(blueOakHeader);
  if (packages.length != expectedBlueOakLibraries) {
    throw new Error(`BlueOak Licenses have changed: Found ${packages.length}, Expected: ${expectedBlueOakLibraries}`);
  }
  let minimatchIncluded = false;
  packages.forEach((package) => {
    ...
    } else if (PackageVersionPair[0] == 'minimatch') {
      if (!minimatchIncluded) {
        console.log(minimatchTemplate);
        minimatchIncluded = true;
      }
    } else if (PackageVersionPair[0] == 'minipass') {
      console.log(minipassTemplate.replace(versionPlaceholder, PackageVersionPair[1]));
    } else {
